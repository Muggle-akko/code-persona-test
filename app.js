(function () {
  ...
  var questions = [
    // Q01 | 维度01 稳定交付感：测“面对未定约定时，能否主动把事情收束成可执行安排”。
    // 给分：a=3 主动定时间地点并闭环；b=2 给大方向但继续开放；c=1 先答应，落地留到现场。
    q(0, "朋友约周末出门，但地点时间都没定，你会：", [
      ["a", "先拉个小群，把时间地点定下来。", 3],
      ["b", "说个大概方向，等大家慢慢补充。", 2],
      ["c", "先答应，具体到时候再说。", 1]
    ]),
    // Q02 | 维度01 稳定交付感：测“承诺遇到阻塞时，是否仍然推进交付结果”。
    // 给分：a=2 会沟通替代方案；b=3 直接完成承诺闭环；c=1 被阻塞卡住、开始摇摆。
    q(0, "你答应帮人带杯奶茶，走到店门口发现排长队：", [
      ["a", "先问对方能不能换一家，别让人空等。", 2],
      ["b", "排，买，送到，任务闭环。", 3],
      ["c", "发一句“人好多”，然后开始犹豫。", 1]
    ]),
    // Q03 | 维度02 错误恢复力：测“社交小事故发生后，能否快速修复并回到在线状态”。
    // 给分：a=3 撤回、道歉、补救；b=2 先观察再行动；c=1 短暂宕机逃离现场。
    ...
    // Q05 | 维度03 过载阈值：测“请求洪峰出现时，能否排队、分级、控制吞吐”。
    // 给分：a=3 按优先级排队处理；b=2 先处理一部分；c=1 被红点压垮并回避。
    ...
    // Q09 | 维度05 权限边界感：测“私人资源/账号是否默认开放，以及授权意识强弱”。
    // 给分：a=1 直接开放；b=2 按关系和用途临时授权；c=3 保护私人账号并给替代方案。
    q(4, "朋友问你借视频会员账号，你会：", [
      ["a", "直接给，反正朋友嘛。", 1],
      ["b", "看关系和用途，临时给一下可以。", 2],
      ["c", "不太想共享私人账号，会推荐拼车或别的办法。", 3]
    ]),
    // Q10 | 维度05 权限边界感：测“别人把额外工作默认塞给你时，是否区分帮忙和归属”。
    // 给分：a=1 默认接下并自我透支；b=2 先澄清优先级；c=3 明确可帮范围和责任边界。
    ...
    // Q14 | 维度07 需求解析力：测“长文本/公告中提取执行信息的能力”。
    // 给分：a=3 抽取时间地点动作；b=2 先看大概再回找；c=1 收藏代替处理。
    q(6, "看一篇很长的群公告，你通常：", [
      ["a", "扫出时间、地点、要做什么。", 3],
      ["b", "看个大概，重要的再回头找。", 2],
      ["c", "先收藏，收藏等于看过。", 1]
    ]),
    // Q15 | 维度08 抽象理解力：测“面对新规则系统时，是理解机制、边玩边学，还是依赖别人”。
    // 给分：a=3 先理解底层机制；b=2 实践中学习；c=1 依赖他人解释。
    q(7, "玩一个新桌游，规则书有三页，你会：", [
      ["a", "先理解机制，不然玩着没安全感。", 3],
      ["b", "边玩边学，错了再说。", 2],
      ["c", "谁会谁教我，我负责气氛。", 1]
    ]),
    // Q16 | 维度08 抽象理解力：测“遇到新现象时，是否追溯来源和传播机制”。
    // 给分：a=3 追问来源和成因；b=2 会用即可；c=1 理解滞后、低兴趣。
    q(7, "刷到一个新互联网热梗，你更像：", [
      ["a", "马上想知道它从哪来、为什么火。", 3],
      ["b", "会用就行，别解释，解释就不灵了。", 2],
      ["c", "等它过气了我可能刚看懂。", 1]
    ]),
    // Q17 | 维度09 不确定耐受度：测“计划突变后，能否在信息不完整时继续行动”。
    // 给分：a=3 快速找下一站；b=2 慌但可改路；c=1 计划消失后明显失稳。
    q(8, "旅行到了陌生城市，计划突然变了：", [
      ["a", "没事，打开地图找下一站。", 3],
      ["b", "有点慌，但能改路线。", 2],
      ["c", "计划没了，我也快没了。", 1]
    ]),
    // Q18 | 维度09 不确定耐受度：测“面对开放式承诺时，对模糊未来的接受程度”。
    // 给分：a=3 接受不确定并先推进当前步；b=2 记挂但能等；c=1 强烈需要明确答案。
    q(8, "别人说“之后再看情况”，你通常：", [
      ["a", "可以接受，先把眼前一步走完。", 3],
      ["b", "会记在心里，等一个更准的说法。", 2],
      ["c", "很难受，我想要明确答案。", 1]
    ]),
    // Q19 | 维度10 创建驱动力：测“空白时间出现时，是否自然想从无到有做点东西”。
    // 给分：a=3 主动启动小项目；b=2 看灵感和心情；c=1 保持空白、不启动。
    q(9, "周末突然空出来半天，你会：", [
    ...
    // Q21 | 维度11 重构倾向：测“看到旧系统混乱时，是否想整理、分类、重建路径”。
    // 给分：a=2 暂时忍到集中整理；b=3 立刻分区标记；c=1 关闭问题、维持现状。
    q(10, "看到家里抽屉乱成一团，你会：", [
      ["a", "先忍，等哪天爆发式整理。", 2],
      ["b", "很想立刻分区贴标签。", 3],
      ["c", "抽屉关上，世界和平。", 1]
    ]),
    // Q22 | 维度11 重构倾向：测“面对沉积数据/杂物时，是否主动清理、去重、分类”。
    // 给分：a=2 偶尔小范围整理；b=3 系统清理分类；c=1 保持原状不介入。
    ...
    // Q27 | 维度14 沟通转译力：测“冲突双方其实不在说同一件事时，能否翻译底层诉求”。
    // 给分：a=3 主动转译双方真实意思；b=2 先降温提醒；c=1 潜水回避。
    ...
    // Q29 | 维度15 表达真实度：测“群体气氛和个人真实评价冲突时，是否表达真实感受”。
    // 给分：a=1 隐藏真实想法维持气氛；b=2 看关系场合表达；c=3 温和但诚实表达。
    q(14, "群里大家都在夸一个你觉得一般的东西，你会：", [
      ["a", "不破坏气氛，先不说。", 1],
      ["b", "看场合，熟人才说真话。", 2],
      ["c", "温和但诚实地说：我没那么喜欢。", 3]
    ]),
    // Q30 | 维度15 表达真实度：测“状态不好时，是继续伪装、适度说明，还是直接呈现”。
    // 给分：a=1 继续营业、回家再崩；b=2 说明电量低；c=3 直接承认当前不在线。
    q(14, "你状态不好但还在社交场合里：", [
      ["a", "努力营业，回家再塌。", 1],
      ["b", "会说今天电量低，少说一点。", 2],
      ["c", "直接承认我现在有点不在线。", 3]
